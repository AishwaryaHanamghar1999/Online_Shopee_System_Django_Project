...
class ProductView(View):
	def get(self, request):
		...
		return render(request, 'app/home.html', {'laptop':laptop, 'mobiles':mobiles,'accessories':accessories,'watch':watch, 'totalitem':totalitem})
...
def watch(request, data=None):
    ...
    if data==None :
        watch = Product.objects.filter(category='W')
    elif data == 'Automaticwatch' or data == 'Quartzwatch':
        watch = Product.objects.filter(category='W').filter(brand=data)
    elif data == 'Digitalwatch':
        watch = Product.objects.filter(category='W').filter(brand=data)
    elif data == 'Militarywatch':
        watch = Product.objects.filter(category='W').filter(brand=data)
    # elif data =='Smartwatch':
    #     watch = Product.objects.filter(category="W").filter(brand=data)
    elif data == 'Pocketwatch':
        watch = Product.objects.filter(category='W').filter(brand=data)
    # elif data == 'Sportwatch':
    #     watch = Product.objects.filter(category='W').filter(brand=data)
